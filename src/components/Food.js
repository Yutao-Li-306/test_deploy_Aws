...
const Food = () => {
    ...
    const printDescription = (e, name) => {
        console.log(name)
        if (timesClicked.get(name) == undefined)
            timesClicked.set(name, 0)
        timesClicked.set(name, timesClicked.get(name) + 1)
        console.log(timesClicked.get(name))
                    ...
                    {foods.map((item, index) => (
                        <motion.div
                            ...
                            <div className='flex justify-between px-2 py-4'>
                                ...
                                <Button variant="light" style={divStyle} onClick={(e) => printDescription(e, item.name)}> <BsPlusCircle/></Button>
                            </div>
